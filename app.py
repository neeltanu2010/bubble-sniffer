...
FINANCIFY_TOOL_NAME = "bubble-sniffer"
require_tool_access(FINANCIFY_TOOL_NAME, "Bubble Sniffer")
# =============================================
...
<style>
    ...
    .score-hero {
        ...
        padding: 1.65rem;
        background:
            radial-gradient(circle at 88% 18%, rgba(255,183,3,0.28), transparent 13rem),
            linear-gradient(135deg, rgba(17,24,39,0.97), rgba(31,41,55,0.95));
        color: #ffffff;
    ...
    .score-fill {
        height: 100%;
        border-radius: 999px;
        background: linear-gradient(90deg, #FFE082, #FFB703, #FB8500);
        box-shadow: 0 0 28px rgba(255,183,3,0.38);
    }
    ...
    .share-box {
        ...
        background: linear-gradient(135deg, rgba(255,243,176,0.94), rgba(255,255,255,0.92));
        border: 1px solid rgba(251,133,0,0.18);
        box-shadow: 0 18px 48px rgba(15,23,42,0.10);
        margin: 1rem 0;
    }
...
def positive_and_increasing(values):
    ...
    # Old strict helper kept for compatibility, but the main scorecard now uses
    # smarter quality checks below. Real businesses breathe; one weak year should
    # not automatically destroy an otherwise strong long-term record.
...
def _recent_avg(values, n=3):
    vals = valid_values(values)
    if not vals:
        return np.nan
    return float(np.mean(vals[-min(n, len(vals)) :]))


def _earlier_avg(values):
    vals = valid_values(values)
    if not vals:
        return np.nan
    split = max(1, len(vals) - min(3, len(vals)))
    return float(np.mean(vals[:split]))
...
def _negative_count(values):
    vals = valid_values(values)
    return sum(1 for v in vals if v < 0)


def _not_sharply_falling(values, allowed_drop=0.30):
    vals = valid_values(values)
    if len(vals) < 2:
        return False
    ...
    if avg_val <= 0:
        return latest_val >= vals[0]

    return latest_val >= avg_val * (1 - allowed_drop)


def _long_term_direction_ok(values):
    vals = valid_values(values)
    if len(vals) < 3:
        return False

    recent = _recent_avg(vals, 3)
    earlier = _earlier_avg(vals)
    latest_val = vals[-1]

    if np.isnan(recent) or np.isnan(earlier):
        return False

    # Direction can be healthy if recent average is better than earlier years,
    # or latest value is above the 5-year average. This avoids punishing one dip.
    return recent >= earlier * 0.98 or latest_val >= _avg(vals)


def smart_pat_margin_quality(margins, threshold=20):
    vals = valid_values(margins)
    if len(vals) < 3:
        return 0, "Fail", "PAT margin data is incomplete; need at least 3 usable years."

    latest_val = vals[-1]
    avg_val = _avg(vals)
    recent = _recent_avg(vals, 3)
    earlier = _earlier_avg(vals)
    direction_ok = _long_term_direction_ok(vals)

    if latest_val >= threshold and avg_val >= threshold * 0.75 and direction_ok:
        return 1, "Pass", f"PAT margin is strong at {latest_val:.2f}% and the long-term direction is healthy, without demanding a perfect 5-year staircase."

    if latest_val >= threshold * 0.75 or recent >= earlier:
        return 0.5, "Partial", f"PAT margin is not perfectly clean, but latest/recent margin is still worth studying. Latest: {latest_val:.2f}%."

    return 0, "Fail", f"PAT margin does not show enough strength or healthy long-term direction. Latest: {latest_val:.2f}%."


def smart_return_ratio_quality(roe_values, roce_values, threshold=20):
    roe = valid_values(roe_values)
    roce = valid_values(roce_values)

    if len(roe) < 3 or len(roce) < 3:
        return 0, "Fail", "ROE/ROCE data is incomplete; need at least 3 usable years for both."

    roe_latest, roce_latest = roe[-1], roce[-1]
    roe_avg, roce_avg = _avg(roe), _avg(roce)
    roe_ok = roe_avg >= threshold and roe_latest >= threshold * 0.75 and _not_sharply_falling(roe)
    roce_ok = roce_avg >= threshold and roce_latest >= threshold * 0.75 and _not_sharply_falling(roce)

    if roe_ok and roce_ok:
        return 1, "Pass", f"ROE and ROCE are durable. 5-year averages: ROE {roe_avg:.2f}%, ROCE {roce_avg:.2f}%. One small dip is allowed."

    partial_roe = roe_avg >= 15 and roe_latest >= 12 and _not_sharply_falling(roe, allowed_drop=0.40)
    partial_roce = roce_avg >= 15 and roce_latest >= 12 and _not_sharply_falling(roce, allowed_drop=0.40)

    if partial_roe and partial_roce:
        return 0.5, "Partial", f"Returns are decent but not elite. 5-year averages: ROE {roe_avg:.2f}%, ROCE {roce_avg:.2f}%."

    return 0, "Fail", f"ROE/ROCE do not show enough durable return strength. 5-year averages: ROE {roe_avg:.2f}%, ROCE {roce_avg:.2f}%."


def smart_cash_flow_quality(cfo_values, fcf_values, pat_values=None):
    cfo = valid_values(cfo_values)
    fcf = valid_values(fcf_values)
    pat = valid_values(pat_values or [])

    if len(cfo) < 3 or len(fcf) < 3:
        return 0, "Fail", "CFO/FCF data is incomplete; need at least 3 usable years for both."

    cfo_pos = _positive_count(cfo)
    fcf_neg = _negative_count(fcf)
    cfo_cumulative = float(np.sum(cfo))
    fcf_cumulative = float(np.sum(fcf))
    required_cfo_positive = max(3, min(4, len(cfo)))

    cfo_strength = cfo_pos >= required_cfo_positive and cfo_cumulative > 0
    fcf_strength = fcf_cumulative > 0 and fcf_neg <= 1

    cfo_pat_ok = True
    if len(pat) >= 3:
        common = min(len(cfo), len(pat))
        pat_sum = float(np.sum(pat[-common:]))
        cfo_sum = float(np.sum(cfo[-common:]))
        if pat_sum > 0:
            cfo_pat_ok = cfo_sum >= pat_sum * 0.70

    if cfo_strength and fcf_strength and cfo_pat_ok:
        return 1, "Pass", f"Cash conversion looks healthy: CFO is positive in {cfo_pos}/{len(cfo)} years and cumulative FCF is positive, allowing one normal capex/working-capital dip."

    if cfo_cumulative > 0 and (fcf_cumulative > 0 or cfo_pos >= required_cfo_positive):
        return 0.5, "Partial", f"Cash flow is mixed but not broken: CFO positive years {cfo_pos}/{len(cfo)}, cumulative FCF {'positive' if fcf_cumulative > 0 else 'negative'}."

    return 0, "Fail", f"Cash flow quality is weak: CFO positive years {cfo_pos}/{len(cfo)}, cumulative FCF {'positive' if fcf_cumulative > 0 else 'negative'}."


def smart_incremental_roe_eps_quality(eps_values, incremental_roe_values):
    eps_ok = is_increasing(eps_values)
    inc = valid_values(incremental_roe_values)

    if eps_ok and len(inc) >= 2 and _not_sharply_falling(inc, allowed_drop=0.40):
        return 1, "Pass", "EPS is improving and incremental ROE is not sharply deteriorating. Growth quality looks reasonable."

    if eps_ok:
        return 0.5, "Partial", "EPS is improving, but incremental ROE needs a closer look before calling the growth high quality."

    return 0, "Fail", "EPS and incremental ROE do not confirm enough quality growth."


def smart_margin_inventory_quality(margins, inventory_turnover_values):
    margin_ok = _long_term_direction_ok(margins)
    turnover_ok = _long_term_direction_ok(inventory_turnover_values)

    if margin_ok and turnover_ok:
        return 1, "Pass", "PAT margin and inventory turnover show healthy long-term direction. One small dip is not treated as failure."

    if margin_ok or turnover_ok:
        return 0.5, "Partial", "Either PAT margin or inventory turnover has a healthy long-term direction. The other needs deeper checking."

    return 0, "Fail", "PAT margin and inventory turnover do not show enough healthy long-term direction."
...
def value_text(value, suffix="", decimals=2):
...
def build_bubble_results(volume_index, rsi, dma50, dma200, retail_increased, current_pe, peg_ratio, earnings_yield):
    bubble_score = 0
    bubble_results = []
    ...
    return bubble_score, bubble_df


def calculate_financify_score(fundamental_score, bubble_score):
    quality_part = (float(fundamental_score) / 9) * 70
    anti_bubble_part = (max(0, 7 - float(bubble_score)) / 7) * 30
    score_100 = int(round(max(0, min(100, quality_part + anti_bubble_part))))

    if score_100 >= 82:
        grade = "A"
        label = "Strong honey, low buzz"
    elif score_100 >= 68:
        grade = "B"
        label = "Good business smell, watch valuation"
    elif score_100 >= 52:
        grade = "C"
        label = "Mixed hive, needs deeper study"
    elif score_100 >= 35:
        grade = "D"
        label = "Too many weak cells"
    else:
        grade = "E"
        label = "Bubble smell dominates"

    return score_100, grade, label


def make_warren_view(fundamental_score, bubble_score, current_pe, peg_ratio, earnings_yield, debt_to_equity_analytical):
    overvalued = (
        (not np.isnan(current_pe) and current_pe > 35)
        or (not np.isnan(peg_ratio) and peg_ratio > 1.8)
        or (not np.isnan(earnings_yield) and earnings_yield < 4)
    )
    clean_debt = latest(debt_to_equity_analytical) < 0.2

    if fundamental_score >= 8 and bubble_score <= 2 and clean_debt and not overvalued:
        title = "Likely study deeper, not blindly buy"
        text = "The numbers smell like quality. He would still ask for a moat, honest management, predictable cash flows and a margin of safety before touching the buy button."
    elif fundamental_score >= 8 and overvalued:
        title = "Great business, maybe not a great price"
        text = "He may admire the company but wait patiently. A wonderful business can still become a poor investment when the entry price already assumes perfection."
    elif fundamental_score >= 6 and bubble_score <= 3:
        title = "Watchlist candidate"
        text = "He would probably keep it in the study pile, read annual reports, compare competitors and wait for a better price instead of chasing the candle."
    elif bubble_score >= 5:
        title = "Avoid the crowd noise"
        text = "Too much excitement and too little safety. He would likely ignore the buzz and look for a calmer opportunity with better downside protection."
    else:
        title = "Pass until proof improves"
        text = "The business has not shown enough consistent honey yet. He would rather miss a noisy opportunity than lose money in a weak one."

    return title, text


def make_action_note(financify_score, bubble_score):
    if financify_score >= 82 and bubble_score <= 2:
        return "Study Zone", "Looks interesting enough for deeper research: moat, management, valuation history, and future growth durability."
    if financify_score >= 68:
        return "Watchlist Zone", "Numbers are decent, but wait for stronger confirmation or a better valuation cushion."
    if bubble_score >= 5:
        return "Bee Careful Zone", "The crowd may be louder than the cash flows. Avoid emotional entry and demand a big margin of safety."
    return "Research More Zone", "The result is mixed. Do not make a decision from one score; compare peers and read the annual report."


def build_share_text(symbol, financify_score, grade, label, fundamental_score, bubble_score, warren_title):
    symbol = str(symbol or "STOCK").upper().strip()
    return (
        f"🐝 Financify Bubble Sniffer Result for {symbol}\n"
        f"Financify Score: {financify_score}/100 ({grade}) — {label}\n"
        f"Fundamental Honey: {score_text(fundamental_score)}/9 | Bubble Smell: {score_text(bubble_score)}/7\n"
        f"What Warren Would Do: {warren_title}\n"
        "Educational only. Not investment advice."
    )


def render_financify_score_panel(symbol, financify_score, grade, label, fundamental_score, bubble_score, action_title, action_text):
    symbol_html = _safe_text(str(symbol or "Stock").upper())
    label_html = _safe_text(label)
    action_title_html = _safe_text(action_title)
    action_text_html = _safe_text(action_text)
    meter_width = max(0, min(100, int(financify_score)))

    st.markdown(
        f"""
        <div class="score-hero">
            <div class="score-kicker">⚡ FINANCIFY SCORE • {symbol_html}</div>
            <div class="score-number">{financify_score}<span style="font-size:32%; letter-spacing:-1px; color:#FFE8A3;">/100</span></div>
            <div class="score-label">Grade {grade} — {label_html}</div>
            <div class="score-meter"><div class="score-fill" style="width:{meter_width}%;"></div></div>
        </div>
        <div class="hook-grid">
            <div class="hook-card">
                <div class="hook-card-title">🍯 Fundamental Honey</div>
                <div class="hook-card-text">{score_text(fundamental_score)}/9 quality checks passed. This is the business-strength side of the score.</div>
            </div>
            <div class="hook-card">
                <div class="hook-card-title">🐝 Bubble Smell</div>
                <div class="hook-card-text">{score_text(bubble_score)}/7 hype-risk points detected. Lower is cleaner.</div>
            </div>
            <div class="hook-card">
                <div class="hook-card-title">🎯 {action_title_html}</div>
                <div class="hook-card-text">{action_text_html}</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True
...
def render_warren_card(title, text, current_pe, peg_ratio, earnings_yield):
    title_html = _safe_text(title)
    text_html = _safe_text(text)
    st.markdown(
        f"""
        <div class="hook-grid">
            <div class="hook-card">
                <div class="hook-card-title">🧠 What Warren Would Do</div>
                <div class="hook-card-text"><b>{title_html}</b><br>{text_html}</div>
            </div>
            <div class="hook-card">
                <div class="hook-card-title">🛡️ Margin of Safety Lens</div>
                <div class="hook-card-text">P/E: {value_text(current_pe)} • PEG: {value_text(peg_ratio)} • Earnings yield: {value_text(earnings_yield, '%')}</div>
            </div>
            <div class="hook-card">
                <div class="hook-card-title">🐝 Human Reminder</div>
                <div class="hook-card-text">Even a good score is only a starting filter. Read the annual report before trusting the buzz.</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True
...
def render_share_panel(symbol, share_text):
    share_html = _safe_text(share_text)
    clean_symbol = re.sub(r"[^A-Za-z0-9_-]+", "_", str(symbol or "stock").strip()) or "stock"
    encoded = quote_plus(share_text)

    st.markdown(
        f"""
        <div class="share-box">
            <div class="share-title">📤 Share-ready result</div>
            <div class="share-text">{share_html}</div>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    with c1:
        st.download_button(
            "📥 Download Share Note",
            data=share_text,
            file_name=f"financify_{clean_symbol}_bubble_sniffer.txt",
            mime="text/plain",
    ...
    with c3:
        st.link_button("𝕏 Share on X", f"https://twitter.com/intent/tweet?text={encoded}", use_container_width=True)
...
st.markdown(
    ...
    <div class="hero-card">
        ...
        <div class="hero-subtitle">
            A sticky, share-worthy stock dashboard that turns boring fundamentals into a Financify Score, bubble smell test, and Warren-style decision note. Educational only. Not investment advice.
        </div>
        <div class="mini-pill-row">
            <div class="mini-pill">⚡ Financify Score</div>
            <div class="mini-pill">🧠 What Warren Would Do</div>
            <div class="mini-pill">📤 Shareable Result</div>
...
dialogue_box(
    ...
    "Enter the Screener/NSE symbol and Yahoo Finance symbol, then press Sniff Bubble. You now get a hook-ready Financify Score, Warren-style decision note, bubble smell meter and share-ready result text.",
    "honey"
)
...
if run:
    ...
    dialogue_box(
        "Quality first",
        "This section checks durable business direction, return strength and cash conversion without demanding a perfect year-by-year staircase. Real businesses breathe; repeated weakness still gets punished.",
        "green"
    )
    ...
    pat_points, pat_result, pat_comment = smart_pat_margin_quality(profit_after_tax_margin)
    score += pat_points
    results.append(["Profit After Tax Margin", pat_result, pat_comment])
    ...
    cash_points, cash_result, cash_comment = smart_cash_flow_quality(
        cash_flow_from_operations,
        free_cash_flow,
        profit_after_tax
    )
    score += cash_points
    results.append(["Cash Flow Quality", cash_result, cash_comment])

    return_points, return_result, return_comment = smart_return_ratio_quality(
        return_on_equity,
        return_on_capital_employed
    )
    score += return_points
    results.append(["Return Ratios", return_result, return_comment])

    inc_points, inc_result, inc_comment = smart_incremental_roe_eps_quality(
        earnings_per_share,
        incremental_roe
    )
    score += inc_points
    results.append(["Incremental ROE vs EPS", inc_result, inc_comment])

    margin_inventory_points, margin_inventory_result, margin_inventory_comment = smart_margin_inventory_quality(
        profit_after_tax_margin,
        inventory_turnover
    )
    score += margin_inventory_points
    results.append(["PAT Margin vs Inventory Turnover", margin_inventory_result, margin_inventory_comment])
    ...
    c1.metric("Fundamental Score", f"{score_text(score)}/9")
    ...
    bubble_score, bubble_df = build_bubble_results(
        volume_index=volume_index,
        rsi=rsi,
        dma50=dma50,
        dma200=dma200,
        retail_increased=retail_increased,
        current_pe=current_pe,
        peg_ratio=peg_ratio,
        earnings_yield=earnings_yield
    )

    financify_score, financify_grade, financify_label = calculate_financify_score(score, bubble_score)
    action_title, action_text = make_action_note(financify_score, bubble_score)
    warren_title, warren_text = make_warren_view(
        fundamental_score=score,
        bubble_score=bubble_score,
        current_pe=current_pe,
        peg_ratio=peg_ratio,
        earnings_yield=earnings_yield,
        debt_to_equity_analytical=debt_to_equity_analytical
    )

    section_title("⚡ Financify Hook Score")
    render_financify_score_panel(
        symbol=screener_symbol,
        financify_score=financify_score,
        grade=financify_grade,
        label=financify_label,
        fundamental_score=score,
        bubble_score=bubble_score,
    ...
    if score >= 8:
        dialogue_box(
            "🍯 Strong Honey",
            "Fundamental score is 8 or above. The business numbers look clean, but valuation and margin of safety still matter.",
            "green"
        )
    else:
        dialogue_box(
            "🐝 Fundamental score is below 8",
            "Business quality is not clean enough yet, even after using the smarter relaxed scorecard. Bubble / hype indicators now become more important.",
            "honey"
        )

    section_title("🐝 Bubble / Hype Check")

    st.metric("Bubble Score", f"{score_text(bubble_score)}/7")

    if bubble_score >= 5:
        dialogue_box("🔥 Full Madness Mode", "Strong hype signs are visible. Bubble risk looks high, especially if fundamentals are weak.", "red")
    elif bubble_score >= 3:
        dialogue_box("🐝 Buzz Building", "Some bubble signs are visible. Be careful with emotional entries and valuation comfort.", "honey")
    else:
        dialogue_box("🍯 No Major Bubble Smell", "Hype signs are not very strong based on the available data.", "green")

    show_numbered_dataframe(bubble_df)

    section_title("🧠 What Warren Would Do")
    render_warren_card(warren_title, warren_text, current_pe, peg_ratio, earnings_yield)

    section_title("📤 Share Button + Viral Result Text")
    share_text = build_share_text(
        ...
        fundamental_score=score,
        bubble_score=bubble_score,
        warren_title=warren_title
    )
    render_share_panel(screener_symbol, share_text)
