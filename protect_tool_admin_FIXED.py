...
# Admin emails bypass monthly usage limits after OTP login.
# Keep emails lowercase. Add more emails here if needed later.
ADMIN_EMAILS = {
    "neeltanu2010@gmail.com",
}
...
def record_tool_use(tool_name: str):
    ...
    if is_admin_email(email):
        return {"ok": True, "admin": True, "message": "Admin usage not counted."}
...
def require_tool_access(tool_name: str, display_name: str = None):
    ...
    if plan == "admin":
        st.success("👑 Admin access active — usage is not counted for this email.")
